...
- **Stack** — the set of **components** that together describe the target application. Each component captures one aspect of the implementation a sensor needs to know about in order to observe the right thing: a technology (language, runtime), framework, observability surface (metrics, logs, traces), messaging/queue, datastore, deployment target, etc. The stack is also where **journeys** are declared. Persisted as `stack.yaml`; produced by `/detect-sensors` Phase A.
- **Journey** — a coherent slice of application behavior declared on the stack. Each journey groups one or more **usecases**.
- **Usecase** — one observable variation of a journey: a `trigger` (narrative + input fixture), expected `behavior`, and `expected_outcome` (invariants + side_effects, with output fixture when applicable). Fixtures and invariants are the acceptance criteria a sensor validates against. Persisted as `<project>/.harness/usecases/<journey-id>/<usecase-id>.yaml`.
- **Sensor** — a reusable feedback control. Observes the system *after* the agent acts and emits Signals optimized for self-correction. A sensor assembles steps and commands against the stack's components to deterministically validate one or more usecases; sensors are **building blocks**, designed to be aggregated, composed, and chained so the same usecase can be checked from different angles (e.g., probing metrics after a deploy, running the build, replaying an HTTP request and asserting on logs).
...
11. **Test data and test helpers are split by purpose.** Three locations, three purposes:
    ...
    - `.harness/fixtures/<name>` — sensor-domain fixture data, referenced by sensor steps via `with: { fixture: <name> }` or interpolation `${{ fixtures.<name> }}`. NOT a Go test fixture. Lives in the user project tree (under `.harness/`) and is consumed at sensor runtime. Sub-paths permitted. Sensors declare which usecases they validate via the top-level `use_cases[]` traceability list; the runtime does not auto-replay those entries.
...
13. **Sensors are composable, stack-driven building blocks.** A sensor's steps and commands MUST be derived from the stack's components — a sensor that asserts on metrics presupposes a metrics component on the stack, one that runs the build presupposes a build component, one that replays HTTP and inspects logs presupposes an HTTP surface plus a log component, and so on. Sensors are NOT allowed to hardcode framework- or technology-specific assumptions that are not represented by a component; if the component is missing, the sensor is not applicable, not "best-effort". Every sensor MUST declare the usecase ids it validates via the top-level `use_cases[]` traceability list. Composition is the default: validating one usecase from multiple angles (build + metrics + HTTP replay + log assertion) is expressed by chaining independent, narrowly-scoped sensors via `depends_on` — never by widening one sensor to do everything. Two sensors that observe the same usecase through different components are healthier than one sensor that fuses them.
...
- `schemas/sensor.yaml` — the definition contract. References `signal.yaml` two ways:
  ...
  - The enum site inside `sensor.yaml` (`execution.exit_code_map[].{verdict,severity}`) uses `{ "$ref": "signal.yaml#/$defs/Verdict" }` and `…/Severity`. Adding a new verdict or severity value means editing `signal.yaml` only — `sensor.yaml` picks it up automatically.
