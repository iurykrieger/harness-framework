...
1. **`HARNESS_REGISTRY_ROOT` env var.** If set and non-empty, must be an absolute path to an existing directory. The env var is the explicit override — it always wins over filesystem walk-up. Returns `Source = SourceEnv`.
2. **Walk-up from `startDir` looking for a `sensors/` directory.** Starts at `filepath.Abs(startDir)` and walks parent-by-parent until either:
   - A directory containing `sensors/` (and `sensors/` is itself a directory) is found → return that ancestor as `projectRoot`, `Source = SourceWalkUp`.
...
This message is generated by a small helper `registry.DiscoveryErrorSignal(err)` exported from `lib/registry/root.go` to keep the four skills consistent.
...
- [ ] `HARNESS_REGISTRY_ROOT` is documented in `CLAUDE.md` (Architecture → "The two schemas" or a new "Registry root discovery" subsection).
