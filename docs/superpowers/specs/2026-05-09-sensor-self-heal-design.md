...
`persist-sensor.go` is a thin CLI wrapper. The validate-and-write core lives in `lib/sensor/persist.go` (extracted from `skills/detect-sensors/scripts/write-sensor.go` as Phase 5 — see Implementation phases). `lib/heal/persist.go` is a similarly thin wrapper that adds heal-specific concerns (atomic batched writes when a Plan has multiple sensor mutations, version bump on patch) on top of `lib/sensor/persist.go`. Concretely:

```
skills/detect-sensors/scripts/write-sensor.go      → CLI wrapper, calls lib/sensor/persist.go
skills/heal-sensor/scripts/persist-sensor.go        → CLI wrapper, calls lib/heal/persist.go
lib/heal/persist.go                                 → batched/version-aware, calls lib/sensor/persist.go
lib/sensor/persist.go                               → schema validation + atomic write (the actual core)
```

Both skills end up funneling through `lib/sensor/persist.go` for the validator-and-write step. No duplication; the heal-specific layer is real (batch + version bump), so it lives where it's used.
...
1. **Phase 1 — `lib/heal/` foundations.** `plan.go`, `patterns.go`, `classify.go`, `apply.go`, `envwriter.go`, `persist.go` (thin wrapper) with full test coverage. No skill or hook yet. Verifiable by tests alone. Depends on Phase 5's `lib/sensor/persist.go` extraction; either land Phase 5 first or stub `lib/sensor/persist.go` here and complete the extraction in 5.
