---
name: list-sensors
description: Use when the user invokes /list-sensors or asks "what's running". No arguments. Reads `.harness/runtime/running_sensors.json`, validates each entry's PID with `kill(pid, 0)`, and emits a single Signal `verdict=pass`, `metadata.kind=list`, `metadata.entries=[…]`. Each entry carries sensor_id, pid (with pid_alive flag), watcher_pid (with watcher_alive), started_at, command, held_by (each holder annotated with its own pid_alive when kind=sensor), signals_log_path, and state ("running" or "orphan" when the subprocess pid is dead).
---
