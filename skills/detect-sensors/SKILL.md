...
- **For `kind=observation` + `output=stream` sensors (Phase B):** do NOT hand-craft regexes. Instead:
  ...
  2. Filter `log_shapes[]` to the shapes relevant to the sensor's command. For `run-*` / `watch-*` sensors observing a running service, that typically means shapes produced by components with role `logger`, `log-encoder`, `http-server`, `http-router`, or `http-middleware`. For `tail-*` / `fetch-*` sensors against external log stores, pick the shape whose encoder matches what the store emits.
  3. For each selected shape, derive patterns from TWO orthogonal axes (severity classes AND HTTP request structure). A shape may satisfy one, the other, or both — emit the union, and order them severity-first so a logger-labelled crash beats the per-request catch-all when both match the same line. Aim for 2–6 patterns per shape total (don't fan out beyond that).
     ...
     - **HTTP-request patterns** (when the shape's `format == "combined-log-format"`, OR any `fields[].meaning ∈ {status_code, method, path, latency_ms}`, OR any `produced_by[]` component has role `http-server` / `http-router` / `http-middleware`). These exist so observation sensors emit one Signal per HTTP request — the *core* observability surface of an http-api project. The status-code class IS the authority on the per-request verdict, regardless of which severity the middleware chose to label the line with. Emit exactly four patterns, one per status class:
       - status code `2\d\d` → `verdict: pass, severity: info` (successful request — the dense observation channel).
       - status code `3\d\d` → `verdict: pass, severity: info` (redirect; still healthy).
       - status code `4\d\d` → `verdict: warn, severity: low` (client error; noteworthy but not a service-health failure).
       - status code `5\d\d` → `verdict: fail, severity: high` (server error; service-health regression).

       Anchor the regex on the shape's actual encoding so it cannot drift away from reality:
       - For `format: combined-log-format` (Apache/Nginx access log): `'^\S+ \S+ \S+ \[[^\]]+\] "(\S+) (\S+)[^"]*" (2\d{2}) '` and the same with `(3\d{2})`/`(4\d{2})`/`(5\d{2})`. Use `captures.{excerpt: 2, rationale: 3}` so the Signal evidence carries the path and the status code.
       - For `format: json` / `logfmt` with a structured `status_code` field — use the **literal key** from `fields[].key` (it might be `status`, `status_code`, `statusCode`, `res.statusCode`, etc.; consult the shape, do not assume). Example for a key literally named `status_code`: `'"status_code"\s*:\s*(2\d{2})\b'`. When `method` and `path` keys are also present in the shape, add their `key` literals to the regex so the captured `excerpt`/`rationale` is informative.
       - For shapes where `status_code` only appears embedded in a free-form `msg` string (e.g. a Zap line whose `msg` literally contains `"GET /foo HTTP/1.1\" from 127.0.0.1 - 200 12B in 3ms"`), anchor on that embedded substring shape — typically `'"msg":"[^"]*?\\"(\S+) (\S+) HTTP[^\\\"]*\\" [^"]*? (2\d{2}) '` and class variants. When this is the only viable anchor, note it in `blind_spots[]` because the regex is sensitive to the middleware's logging format string.
  4. Anchor every drafted regex on the shape's `sample`: the regex MUST match the sample. If it doesn't, the regex is wrong. For HTTP-request patterns, either the shape's `sample` already contains a successful request line (in which case the 2xx variant must match it), or the discovery step persisted only a non-request sample — in that case capture or synthesize a representative request line and either widen the shape's `sample` (re-running `/detect-sensors --refresh-stack`) or accept that only one of the four status-class regexes can be sample-anchored. Never ship a regex whose sample-anchor was waived silently — call it out in `blind_spots[]`.
  5. In the sensor's `description`, cite the source: e.g. *"output_parsing derived from log_shape 'zap-prod-json' in .harness/stack.yaml"* or *"derived from log_shapes 'chi-access-log' (HTTP per-request observation) and 'zap-prod-json' (severity tiers)"*. This is the audit trail when patterns later fail to match real stdout.
