...
- Logs are append-only; nothing is rotated. Long-running sessions should periodically `/stop-sensor`/`/start-sensor` to keep `.harness/runtime/<id>/` from growing unboundedly.
